...
function startTimer(secs) {
    ...
    setInterval(function () {
        ...
        if (
            timeMinutes.textContent % 10 == "1" &&
            timeMinutes.textContent != "11"
        ) {
            timeMinutesText.textContent = "минута";
        } else if (
            timeMinutes.textContent != "12" &&
            timeMinutes.textContent != "13" &&
            timeMinutes.textContent != "14" &&
            (timeMinutes.textContent % 10 == "2" ||
            timeMinutes.textContent % 10 == "3" ||
        ...
        if (
            timeSeconds.textContent % 10 == "1" &&
            timeSeconds.textContent != "11"
        ) {
            timeSecondsText.textContent = "секунда";
        } else if (
            timeSeconds.textContent != "12" &&
            timeSeconds.textContent != "13" &&
            timeSeconds.textContent != "14" &&
            (timeSeconds.textContent % 10 == "2" ||
            timeSeconds.textContent % 10 == "3" ||
...
for (let variable of document.querySelectorAll(".figure")) {
    variable.onclick = function () {
        this.querySelector(".figure .figure__hover-content").style.opacity =
            "1";
        setTimeout(
            () =>
                (this.querySelector(
                    ".figure .figure__hover-content"
                ).style.opacity = "0"),
            1000
        );
    };
